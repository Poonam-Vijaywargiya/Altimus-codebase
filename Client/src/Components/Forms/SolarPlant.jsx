...
const SolarPlant = () => {
    ...
    const updateUserData = (e, key) =>{
        ...
        currdata.formData[1].data[key] = e.target.value;
        setUserData(currdata)
    }
    const addMoreSolarPlants = () =>{
        setShowNewFields(true);
    }
    const addMoreFields = (e) =>{
        const currdata = JSON.parse(JSON.stringify(userData));
        currdata.formData[1].data[fieldLabel] =fieldValue;
        setUserData(currdata)
  ...
  return (
    <div className='individual-step'>
        {Object.keys(userData.formData[1].data).map(v =>(
            <TextField
            margin="dense"
            type="number"
            id={v}
        ...
        sx={{ mt: 2, mb: 2 }}><AddBoxIcon onClick={addMoreSolarPlants}/></Button> 
